# ==============================================================================
#  Vxmpire — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Vxmpire compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Vxmpire dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          VXMPIRE  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Vxmpire-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Vxmpire ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Vxmpire depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$VxmpireRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Vxmpire-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "vxmpire-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'vxmpire-dist.zip' introuvable dans la release $version.`n           Contactez le support Vxmpire."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "vxmpire-dist.zip"
    ...
    Write-OK "Vxmpire $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Vxmpire.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Vxmpire. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Vxmpire installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Vxmpire.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez vxmpire-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
