...
// ── CRITIQUE : userData = dossier Vxmpire pour les settings/plugins
const vxmpireData = path.join(app.getPath("appData"), "Vxmpire");
app.setPath("userData", vxmpireData);

// AppUserModelId unique — Windows reconnaît Vxmpire comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Vxmpire sont bloqués.
        ...
        function patchWebContents(wc) {
            ...
            if (wc._vxmpirePatched) return;
            wc._vxmpirePatched = true;
            ...
            wc.setWindowOpenHandler(({ url }) => {
                ...
                console.log("[Vxmpire][LINK] Ouverture externe:", url);
                return { action: "deny" };
            });
            ...
            wc.on("did-create-window", (childWin) => {
                ...
                if (childWc._vxmpirePatched) return;
                childWc._vxmpirePatched = true;
                ...
                childWc.on("will-navigate", (event, url) => {
                    if (!isDiscordUrl(url)) {
                        ...
                        shell.openExternal(url).catch(() => {});
                        console.log("[Vxmpire][CHILD-NAV] Redirection externe:", url);
                ...
                childWc.on('did-navigate', function(_event, url) {
                    if (!isDiscordUrl(url)) {
                        ...
                        console.log('[Vxmpire][CHILD-DID-NAV] Redirection externe apres navigation:', url);
                        try { childWin.close(); } catch (_) {}
                    }
                });
                ...
                childWc.setWindowOpenHandler(({ url }) => {
                    ...
                    shell.openExternal(url).catch(() => {});
                    console.log("[Vxmpire][CHILD-LINK] Ouverture externe:", url);
                    return { action: "deny" };
                });
                ...
                childWc.on("did-finish-load", () => {
                    ...
                    if (url && url !== "about:blank" && !isDiscordUrl(url)) {
                        shell.openExternal(url).catch(() => {});
                        console.log("[Vxmpire][CHILD-LOAD] Fermeture et redirection:", url);
                        try { childWin.close(); } catch (_) {}
                    }
                });
            });
            ...
            wc.on("will-navigate", (event, url) => {
                ...
                if (url !== currentUrl && !isDiscordUrl(url)) {
                    event.preventDefault();
                    shell.openExternal(url).catch(() => {});
                    console.log("[Vxmpire][NAV] Redirection externe:", url);
                }
            });
        }
        ...
        console.log("[Vxmpire] Patch liens externes activé sur TOUS les webContents (avec did-create-window) ✓");
        ...
        app.once("browser-window-created", (_, win) => {
            ...
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Vxmpire] Module bloqué (inutile pour Vxmpire):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Vxmpire] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Vxmpire] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Vxmpire] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(vxmpireData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Vxmpire] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Vxmpire] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Vxmpire] LevelDB check failed:", e.message); }

// Modules bundlés dans vxmpire-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Vxmpire] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Vxmpire] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans vxmpire-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Vxmpire] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
// parent.paths = []) trouvent quand même les modules natifs Vxmpire.
...
console.log("[Vxmpire] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir)) {
        ...
        if (!buildInfo.localModulesRoot) {
            ...
            console.log("[Vxmpire] build_info.json patché → localModulesRoot:", nativeModulesDir);
        }
    }
} catch (e) {
    console.warn("[Vxmpire] Impossible de patcher build_info.json:", e.message);
}
