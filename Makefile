...
all: $(BUILD_DIR)/vxmpire $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/vxmpire_rc.o: vxmpire.rc assets/vxmpire.ico
	mkdir -p $(BUILD_DIR)
	windres vxmpire.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/vxmpire: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/vxmpire.desktop
	rm -f $(PREFIX)/share/pixmaps/vxmpire.png
	rm -f $(PREFIX)/share/pixmaps/vxmpire.svg
	rm -f $(PREFIX)/bin/vxmpire

install: uninstall
	cp vxmpire.desktop $(PREFIX)/share/applications/vxmpire.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/vxmpire.svg
	cp $(BUILD_DIR)/vxmpire $(PREFIX)/bin/vxmpire

run: all
	cp assets/vxmpire.gschema.xml $(BUILD_DIR)/assets/vxmpire.gschema.xml
