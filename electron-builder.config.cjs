...
// ─── Configuration de Build Vxmpire ─────────────────────────────────────────

function killVxmpire() {
    const releaseDir = join(__dirname, "release", "vxmpire-dist");
...
function buildVxmpireFromDiscord(discordApp) {
    ...
    console.log("[vxmpire] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Vxmpire"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "vxmpire", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const vxmpirePreload = join(__dirname, "vxmpire-preload.js");
    if (existsSync(vxmpirePreload)) {
        cpSync(vxmpirePreload, join(outDist, "preload.js"));
    }
    ...
    if (existsSync(macCursorsSrc)) {
        console.log("[vxmpire] Copie des curseurs macOS...");
        cpSync(macCursorsSrc, join(outRes, "mac"), { recursive: true });
    } else {
        console.warn("[vxmpire] ATTENTION: dossier 'mac' introuvable a la racine, le plugin CursorMacOS ne fonctionnera pas dans le build package.");
    }
    ...
    const iconSrc = join(__dirname, "vxmpire.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Vxmpire" --set-version-string "FileDescription" "Vxmpire"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[vxmpire] Build terminé -> ${outDir}`);
}
...
buildVxmpireFromDiscord(discord);

module.exports = {
    appId: "com.vxmpire.app",
    productName: "Vxmpire",
    copyright: "Copyright 2026 Vxmpire",
