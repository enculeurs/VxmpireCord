...
(async () => {
    ...
    // Setup userplugins
    try {
        const userTarget = join(homedir(), "Documents", "Vxmpire", "userplugins");
        ...
        console.error("[Vxmpire] Failed to setup userplugins link", e);
    }
    ...
    const vxmpireFlag = args.includes("--vxmpire");

    let dirs: string[];

    if (vxmpireFlag) {
        dirs = ["src/vxmpireplugins"];
    } else if (equicordFlag) {
        dirs = ["src/vxmpireplugins", "src/userplugins"];
    } else if (vencordFlag) {
        dirs = ["src/plugins", "src/plugins/_core"];
    } else {
        dirs = ["src/plugins", "src/plugins/_core", "src/vxmpireplugins", "src/userplugins"];
    }
