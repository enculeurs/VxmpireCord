...
export const gitHash = process.env.VXMPIRE_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "vxmpireplugins", "vxmpireplugins/_api"];
            ...
            // ─── External User Plugins (~/Documents/Vxmpire/userplugins/) ───────────
            // Scan and auto-create the external userplugins directory.
            const externalUserPluginsDir = join(homedir(), "Documents", "Vxmpire", "userplugins");
            try {
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.VXMPIRE_REMOTE;
            if (!remote) {
...
export const commonOpts = {
    ...
    alias: {
        ...
        "@vxmpireplugins": "./src/vxmpireplugins",
    }
};
