/*
 * Vxmpire — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection Vxmpire présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par Vxmpire
    if (existsSync(appDirPath)) {
        try {
            if (existsSync(join(appDirPath, "index.js"))) {
                ...
                if (!indexContent.includes("Vxmpire Injector") && !indexContent.includes("Vxmpire")) {
                    console.warn(`\x1b[33m[Vxmpire] Le dossier app/ existe mais n'a pas l'air d'avoir été créé par Vxmpire.\x1b[0m`);
        ...
        console.log("[Vxmpire] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[Vxmpire] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[Vxmpire] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[Vxmpire] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[Vxmpire] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Vxmpire] Aucune installation Discord avec Vxmpire injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que Vxmpire a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[Vxmpire] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[Vxmpire] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
