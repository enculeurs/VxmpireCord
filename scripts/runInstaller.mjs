/*
 * Vxmpire â€” Installer via EquilotlCli
 * TÃ©lÃ©charge EquilotlCli.exe depuis les releases Equicord et le lance
 * avec les variables d'environnement pointant vers les fichiers Vxmpire.
 *
 * L'exe affiche une interface graphique permettant de choisir le Discord cible.
 *
 * Usage:
 *   pnpm inject    â†’ installe Vxmpire dans le Discord choisi
 *   pnpm uninject  â†’ dÃ©sinstalle Vxmpire du Discord choisi
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[Vxmpire] Installer already present, using local copy.");
        return outputFile;
    }

    console.log("[Vxmpire] Downloading installer (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: { "User-Agent": "Vxmpire (https://github.com/vxmpirefr/vxmpire)" }
    });
    ...
    console.log("[Vxmpire] Installer downloaded successfully!");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[Vxmpire] dist/desktop/patcher.js not found!\x1b[0m");
        ...
        for (const ver of versions) {
            ...
            if (existsSync(join(base, ver)) && !existsSync(appAsarPath) && !existsSync(backupPath)) {
                try {
                    ...
                    console.log(`[Vxmpire] Supprimé le dossier de mise à jour Discord incomplet : ${join(base, ver)}`);
                } catch (e) {
                    console.warn(`[Vxmpire] Impossible de supprimer ${join(base, ver)}: ${e.message}`);
                }
            }
        }
    }
...
function cleanOldVxmpire(isUninstall) {
    console.log("[Vxmpire] Cleaning previous installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            if (existsSync(appDirPath)) {
                ...
                try {
                    ...
                    if (existsSync(pkgFile)) {
                        ...
                        if (pkg.name === "vxmpire") shouldDelete = true;
                ...
                if (shouldDelete) {
                    ...
                    console.log(`[Vxmpire] Removed legacy app/ folder in ${resourcesDir}`);
                    cleanedAny = true;
                }
            }
            ...
            if (!isUninstall && existsSync(backupPath)) {
                ...
                console.log(`[Vxmpire] Restored _app.asar â†’ app.asar in ${resourcesDir}`);
                cleanedAny = true;
            }

        } catch (e) {
            console.error(`[Vxmpire] Error cleaning ${resourcesDir}:`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[Vxmpire] Cleanup done.");
    } else {
        console.log("[Vxmpire] Nothing to clean.");
    }
}
        ...
        for (const ver of versions) {
            ...
            if (existsSync(backupPath)) {
                ...
                if (existsSync(updateExe)) {
                    console.log(`[Vxmpire] Launching ${channel}...`);
                    ...
                    if (existsSync(directExe)) {
                        console.log(`[Vxmpire] Launching ${channel} (direct)...`);
                        exec(`"${directExe}"`);
                    }
                }
...
cleanOldVxmpire(isUninstall);
if (!isUninstall) checkBuild();

const installerBin = await ensureBinary();

console.log("[Vxmpire] Injecting...");
...
try {
    execFileSync(installerBin, mappedArgs, {
        ...
        env: {
            ...
            VXMPIRE_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[Vxmpire] Injection failed.");
    process.exit(1);
}
