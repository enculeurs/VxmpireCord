...
const getJSON = phin.defaults({
    method: "GET",
    parse: "json",
    headers: {"User-Agent": `Vxmpire Installer/${version}`},
    followRedirects: true
});

/* eslint-disable no-console */
export default async function () {
    const downloadUrl = `https://source.${domain}/api/v1/repos/vxmpire/vxmpire/releases/latest`;
    console.info(`Vxmpire Installer ${version}`);
    ...
    try {
        ...
        if (semverGreaterThan(latestVersion, version)) {
            ...
            const result = await dialog.showMessageBox({
                ...
                message: `A new version of the Vxmpire installer is available. Click "Download" to download the newest version.`,
