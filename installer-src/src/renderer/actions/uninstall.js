...
async function shouldAutoRestart() {
    try {
        const prefsPath = path.join(process.env.APPDATA, "Vxmpire", "settings", "installer-prefs.json");
...
async function deleteShims(paths) {
    ...
    for (const resPath of paths) {
        log(`Removing Vxmpire from: ${resPath}`);
        try {
            ...
            log(`❌ Could not remove Vxmpire from ${resPath}`);
            log(`❌ ${err.message}`);
            return err;
        }
    }
...
export default async function(paths) {
    try {
        ...
        lognewline("Deleting Vxmpire loader and restoring files...");
