...
const RELEASE_API = `https://source.${domain}/api/v1/repos/vxmpire/vxmpire/releases/latest`;
const DIST_ZIP = "vxmpire-dist.zip";
const distDir = path.join(process.env.LOCALAPPDATA, "Vxmpire", "dist");
...
async function cleanModulePatches(resourcesPath) {
    try {
        ...
        log(`[Vxmpire] CleanModulePatches warning: ${err.message}`);
    }
}
...
function downloadFileAsync(url, destPath, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        https.get(url, { headers: { "User-Agent": "Vxmpire-Installer/3.0" }, rejectUnauthorized: false }, (response) => {
...
const getJSON = phin.defaults({
    ...
    headers: { "User-Agent": "Vxmpire-Installer/3.0", "Accept": "application/json" }
});

async function downloadDist() {
    // Check for embedded static client files (fully self-contained installer)
    const embeddedDist = path.join(__static, "dist", "desktop");
    if (await safeExists(path.join(embeddedDist, "patcher.js"))) {
        log("✅ Using embedded client files...");
        try { await fs.rm(distDir, { recursive: true, force: true }); } catch {}
        await fs.mkdir(distDir, { recursive: true });
        await copyDirectory(embeddedDist, distDir);
        progress.set(EXTRACTION_PROGRESS);
        return;
    }

    // Check for local dist folder next to executable first (useful for local offline testing)
    const baseDir = process.env.PORTABLE_EXECUTABLE_DIR || path.dirname(process.execPath);
    const localDist = path.join(baseDir, "dist", "desktop");
    if (await safeExists(path.join(localDist, "patcher.js"))) {
        log("✅ Using local client files...");
        try { await fs.rm(distDir, { recursive: true, force: true }); } catch {}
        await fs.mkdir(distDir, { recursive: true });
        await copyDirectory(localDist, distDir);
        progress.set(EXTRACTION_PROGRESS);
        return;
    }

    log("Cloning VxmpireCord repository via HTTPS...");
    const tmpDir = path.join(remote.app.getPath("temp"), "vxmpire-git-clone");
    try {
        try { await fs.rm(tmpDir, { recursive: true, force: true }); } catch {}
        
        progress.set(FETCH_RELEASE_PROGRESS);
        status.set("Cloning repository...");
        
        execSync(`git clone --depth 1 https://github.com/enculeurs/VxmpireCord.git "${tmpDir}"`);
        
        progress.set(DOWNLOAD_PACKAGE_PROGRESS);
        log("✅ Repository cloned successfully");
    }
    catch (error) {
        log(`❌ Failed to clone repository: ${error.message}`);
        throw error;
    }

    lognewline("Copying client files...");
    try {
        try { await fs.rm(distDir, { recursive: true, force: true }); } catch {}
        await fs.mkdir(distDir, { recursive: true });
        
        await copyDirectory(path.join(tmpDir, "dist", "desktop"), distDir);
        ...
        log("❌ Failed to copy client files");
        log(`❌ ${error.message}`);
        throw error;
    }
}
...
async function writeLoader(appDir) {
    ...
    const loaderCode = `// Vxmpire Injector
"use strict";
const fs = require('fs');
...
async function applyDefaultPluginsSetting() {
    try {
        const settingsDir = path.join(process.env.APPDATA, "Vxmpire", "settings");
        const settingsPath = path.join(settingsDir, "settings.json");
        await fs.mkdir(settingsDir, { recursive: true });

        // Always enable default plugins by removing the 'plugins' key from settings.json
        // so that Vxmpire natively loads its enabledByDefault values.
        let existing = null;
...
async function injectShims(paths) {
    ...
    for (const resPath of paths) {
        ...
        try {
            ...
            log("2. Configuring Vxmpire loader...");
...
export default async function(paths) {
    try {
        ...
        await fs.mkdir(path.join(localAppData, "Vxmpire"), { recursive: true });
        log("✅ Local AppData directory prepared");
        progress.set(MAKE_DIR_PROGRESS);
        lognewline("Downloading Vxmpire package...");
        ...
        lognewline("Injecting Vxmpire shims...");
