...
namespace VxmpireInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private VxmpireBackend _backend;

        public LauncherForm()
        {
            this.Text = "Vxmpire Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("VxmpireInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "VxmpireInstaller_WebView2");
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show(
                        "Microsoft Edge WebView2 Runtime is required to run the Vxmpire Installer but was not found on your system.\n\n" +
                        "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                        "After installing, restart the Vxmpire Installer.",
            ...
            _backend = new VxmpireBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("VxmpireInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("VxmpireInstaller.icon.ico"))
                    {
    ...
    public class VxmpireBackend
    {
        ...
        const string GITEA_REPO = "vxmpire/vxmpire";
        const string GITEA_URL  = "https://source.vxmpire.st";
        const string DIST_ZIP   = "vxmpire-dist.zip";

        public VxmpireBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Vxmpire", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"vxmpire\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but Vxmpire is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already Vxmpire → not a third-party mod
                if (content.Contains("\"vxmpire\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            SetProgress(5, "Extracting embedded client files...");
            if (Directory.Exists(_distDir)) Directory.Delete(_distDir, true);
            Directory.CreateDirectory(_distDir);

            await Task.Run(() =>
            {
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("VxmpireInstaller.dist.zip"))
                {
                    if (stream == null)
                    {
                        throw new Exception("Embedded 'dist.zip' not found in resources.");
                    }
                    var tmpZip = Path.Combine(Path.GetTempPath(), "vxmpire-dist.zip");
                    using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write))
                    {
                        stream.CopyTo(fs);
                    }

                    var tempExtract = Path.Combine(Path.GetTempPath(), "vxmpire-temp-extract");
                    if (Directory.Exists(tempExtract)) Directory.Delete(tempExtract, true);
                    Directory.CreateDirectory(tempExtract);

                    ZipFile.ExtractToDirectory(tmpZip, tempExtract, true);

                    var clonedDist = Path.Combine(tempExtract, "desktop");
                    if (!Directory.Exists(clonedDist))
                    {
                        throw new Exception("Could not find 'desktop' folder in embedded client zip.");
                    }

                    CopyDirectory(clonedDist, _distDir);
        ...
        private void DoInject(string resPath)
        {
            ...
            SetProgress(92, "Configuring Vxmpire loader...");
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            {
                Console.WriteLine($"[Vxmpire] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"vxmpire\""))
                {
        ...
        private void WriteLoader(string appDir)
        {
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"vxmpire\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Vxmpire] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
