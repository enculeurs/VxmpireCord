...
# ── Vérification ─────────────────────────────────────────────────────────────
if [[ ! -f "release/installer/Vxmpire-Installer.exe" ]]; then
    echo ""
    echo " [ERREUR] Vxmpire-Installer.exe introuvable apres build."
    exit 1
fi

SIZE=$(stat -c%s "release/installer/Vxmpire-Installer.exe" 2>/dev/null \
    || stat -f%z "release/installer/Vxmpire-Installer.exe")

echo ""
echo " [OK] Build reussi !"
echo " Fichier : release/installer/Vxmpire-Installer.exe  ($SIZE octets)"
echo ""