// Vxmpire preload — globalPaths fix + Equicord avec contextBridge
"use strict";
    ...
    try {
        ...
        if (rendererJs) {
            webFrame.executeJavaScript(rendererJs).catch(e => {
                console.error("[Vxmpire] renderer inject failed:", e?.message);
            });
        }
    } catch (e) {
        console.error("[Vxmpire] VencordPreloadGetRendererJs failed:", e);
    }
