...
import { t, useTranslation } from "../autoTranslateVxmpire";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("vxmpire_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("vxmpire_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Vxmpire Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `vxmpire_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("vxmpire-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    ...
    stop() {
        ...
        removeHeaderBarButton("vxmpire-event-logs");
