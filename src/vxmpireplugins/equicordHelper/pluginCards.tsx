...
import { EQUIBOT_USER_ID, VXMPIRE_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Vxmpire. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components â€” Vxmpire Bot (domain, Component v2 Container format)
    if (message.author.id === VXMPIRE_BOT_USER_ID) {
