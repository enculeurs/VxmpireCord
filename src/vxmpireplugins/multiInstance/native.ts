...
import { registerMediaPermissionsForSession } from "../../vxmpire/main/mediaPermissions";
...
//
// Chaque instance tourne dans sa propre session Electron (persist:vxmpire-mi-{userId}),
...
const SHARED_SETTINGS_FILE = join(app.getPath("userData"), "vxmpire-mi-shared-settings.json");
...
function saveSharedSettings(settings: Record<string, string>): void {
    try {
        ...
        console.warn("[VxmpireMI] Failed to save shared settings:", e);
    }
}
...
function createTokenPreload(token: string, sharedSettings: Record<string, string> = {}): string {
    ...
    const dir = join(app.getPath("userData"), "vxmpire-mi-preloads");
    ...
    const script = `
// Vxmpire MultiInstance — token preload
...
(function() {
    ...
    try {
        ...
        try {
            ...
            console.warn("[VxmpireMI] Shared settings seed error:", e);
        }

        // Définit le token dans localStorage
        Object.defineProperty(window, '__vxmpire_token', { value: TOKEN, writable: false });
        ...
        console.log("[VxmpireMI] Token preload active ✓");
    } catch(e) {
        console.warn("[VxmpireMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `vxmpire.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:vxmpire-mi-${userId}`;
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Vxmpire [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[VxmpireMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__VXMPIRE_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Vxmpire dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.vxmpire.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[VxmpireMI] arrangeSplit error:", e);
    }
}
