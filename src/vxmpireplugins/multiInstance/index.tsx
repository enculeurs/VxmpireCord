...
import { t } from "../autoTranslateVxmpire";
...
const MI_TOKEN_CACHE_KEY = "vxmpire-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("vxmpire-mi-ctx-root") ?? document.createElement("div");
        el.id = "vxmpire-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("vxmpire-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("vxmpire-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("vxmpire-mi-ctx-root");
        if (root) root.remove();
    },
});
