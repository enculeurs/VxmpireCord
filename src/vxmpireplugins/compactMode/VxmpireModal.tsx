...
import IconsTab from "@vxmpireplugins/iconViewer/components/IconsTab";
...
const TABS: TabDef[] = [
    ...
    },
    {
        ...
        description: "Sync Vxmpire settings across devices",
    },
    {
        id: "language",
        label: "Language",
        ...
        description: "Change Vxmpire's interface language",
    },
];

// ── Modal inner ───────────────────────────────────────────────────────────────

function VxmpireModalInner({ onClose }: { onClose: () => void; }) {
    ...
    let TabContent = null;
    switch (activeTab) {
        case "vxmpire": TabContent = <VencordTab />; break;
...
function VxmpireModal({ modalProps }: { modalProps: ModalProps; }) {
    return (
        <ModalRoot {...modalProps} size={ModalSize.LARGE} className="nc-modal-wrapper">
            <ModalContent className="nc-modal-modal-content">
                <ErrorBoundary message="Failed to render Vxmpire Settings modal.">
                    <VxmpireModalInner onClose={modalProps.onClose} />
                </ErrorBoundary>
            </ModalContent>
        </ModalRoot>
    );
...
export function openVxmpireModal() {
    openModal(modalProps => <VxmpireModal modalProps={modalProps} />);
}
