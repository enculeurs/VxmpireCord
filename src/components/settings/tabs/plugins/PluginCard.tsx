...
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew, hasTutorial }: PluginCardProps) {
    ...
    const openTutorialVideo = (e: React.MouseEvent) => {
        ...
        const videoUrl = `https://git.${domain}/vxmpire/vxmpire-tutorials/raw/branch/main/videos/${encodeURIComponent(videoName)}.mp4`;
        openModal(props => (
    ...
    const isVxmpire = !PluginMeta[plugin.name]?.userPlugin;
    const iconType = isVxmpire ? "vxmpire" : "other";

    // Le système de like ne s'applique qu'aux plugins Vxmpire (pas Vencord/Equicord,
    ...
    const isVxmpireFolderPlugin = PluginMeta[plugin.name]?.folderName?.startsWith("src/vxmpireplugins/") ?? false;
    const canShowLikeBadge = isVxmpireFolderPlugin && !plugin.required && !disabled;
        ...
        openModal(props => (
            <ModalRoot {...props} size={ModalSize.SMALL}>
                ...
                <ModalContent style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px", alignItems: "center" } as any}>
                    {isVxmpire ? (
                        <a href="https://source.vxmpire.st/vxmpire/vxmpire" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "12px", textDecoration: "none", color: "var(--text-normal)", fontSize: "20px", fontWeight: 600 }}>
                            <img src="https://source.vxmpire.st/assets/img/logo.svg" alt="Vxmpire" style={{ width: 64, height: 64, borderRadius: "50%" }} />
                            Vxmpire
                        </a>
                    ) : (
            ...
            infoButton={
                <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    {(plugin.name === "DynamicIslande" || plugin.name === "StereoInstaller" || plugin.name === "ClientDiagnostics" || plugin.name === "SecureBookmarks" || plugin.name === "StatusCycler" || plugin.name === "Surveillance" || plugin.name === "MutualScanner") && (
                        <Tooltip text="This plugin comes from our partner (Illegalcord) then was modified by Vxmpire.">
                            {({ onMouseEnter, onMouseLeave }) => (
                                <button
                                    role="button"
