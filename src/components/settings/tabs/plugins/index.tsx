...
function VxmpireTabIcon() {
    return <img src={VXMPIRE_TAB_ICON} alt="Vxmpire" style={{ width: 18, height: 18, borderRadius: 4 }} />;
}
...
const makeCategoryOptions = (othersCount?: number) => [
    ...
    { label: "Vxmpire", value: SearchStatus.VXMPIRE },
...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Vxmpire"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.VXMPIRE });
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.VXMPIRE:
                if (!pluginMeta?.folderName?.startsWith("src/vxmpireplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (pluginMeta?.folderName?.startsWith("src/vxmpireplugins/") || pluginMeta?.folderName?.startsWith("src/plugins/_")) return false;
    ...
    // Only filter/categorize plugin DATA here — no JSX created yet
        const { vxmpireData, othersData, requiredData } = useMemo(() => {
        const vxmpireData: typeof sortedPlugins = [];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                ...
                if (folderName.startsWith("src/vxmpireplugins/")) {
                    vxmpireData.push(p);
                } else {
                    othersData.push(p);
                }
        ...
        vxmpireData.sort(byLikes);
        othersData.sort(byLikes);

        return { vxmpireData, othersData, requiredData };
    }, [sortedPlugins, searchValue, newPluginsSet, depMap, pluginFilter, ratings]);

    const allDataLength = vxmpireData.length + othersData.length;
    ...
    const vxmpireVisibleData = vxmpireData.slice(0, Math.min(visibleCount, vxmpireData.length));
    const othersVisibleData = othersData.slice(0, Math.max(0, visibleCount - vxmpireData.length));
    ...
    const makeRequiredCard = (p: typeof sortedPlugins[number]) => {
        const tooltipText = p.required || !depMap[p.name]
            ? "This plugin is required for Vxmpire to function."
    ...
    const vxmpirePlugins = vxmpireVisibleData.map(makeCard);
    const othersVisible = othersVisibleData.map(makeCard);
    const requiredPlugins = requiredData.map(makeRequiredCard);

    const totalVxmpirePlugins = React.useMemo(() => {
        return Object.values(Plugins).filter(p => PluginMeta[p.name]?.folderName?.startsWith("src/vxmpireplugins/")).length;
    }, []);

    const totalOtherPlugins = React.useMemo(() => {
        const isVxmpirePlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/vxmpireplugins/");
        const isCorePlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/plugins/_");
        return Object.values(Plugins).filter(p => !isVxmpirePlugin(p.name) && !isCorePlugin(p.name)).length;
    }, []);

    // Category-aware stats for the "ENABLED PLUGINS" card: reflects whichever tab
    // (VXMPIRE / OTHERS / all) is currently selected, instead of always being global.
    const categoryStats = useMemo(() => {
        ...
        if (searchValue.status === SearchStatus.VXMPIRE) {
            plugins = plugins.filter(isVxmpirePlugin);
        } else if (searchValue.status === SearchStatus.OTHERS) {
            plugins = plugins.filter(p => !isVxmpirePlugin(p));
            ...
            plugins = plugins.filter(p => isVxmpirePlugin(p) && ratings[p]?.likedByMe);
        }
        ...
        <SettingsTab>
                ...
                {!premiumOnly && (
                    <div className={cl("ecosystem-banner")}>
                        <div className={cl("ecosystem-banner-text")}>
                            ...
                            <Paragraph>{t("Manage your Vxmpire and community plugins here. Enable, disable, and configure them to your liking.")}</Paragraph>
                        </div>
                            ...
                            onClick={() => {
                                ...
                                if (native?.process?.env) {
                                    ...
                                    if (home) {
                                        ...
                                        const folderPath = isWindows ? `${home}\\Documents\\Vxmpire\\userplugins` : `${home}/Documents/Vxmpire/userplugins`;
                ...
                <div className={classes(Margins.bottom20, cl("filter-controls"))}>
                    ...
                    <div className={cl("filter-buttons")} style={{ minWidth: 220 }}>
                        <SearchableSelect
                            ...
                            value={makeCategoryOptions(totalOtherPlugins).find(o => o.value === searchValue.status)?.value ?? SearchStatus.VXMPIRE}
                            onChange={(v: any) => {
                            ...
                            renderOptionPrefix={(o: any) => {
                                if (o?.value === SearchStatus.VXMPIRE) return <VxmpireTabIcon />;
            ...
            {premiumOnly ? (
                <>
                    ...
                    {vxmpireData.length || othersData.length
                        ? (
                            <div className={cl("grid")}>
                                {[...vxmpirePlugins, ...othersVisible].length
                                    ? [...vxmpirePlugins, ...othersVisible]
                ...
                <>
                    {vxmpireData.length > 0 && searchValue.status === SearchStatus.VXMPIRE && (
                        <div className={cl("grid")}>
                            {vxmpirePlugins}
                        </div>
                    )}
                    ...
                    {searchValue.status === SearchStatus.USER_PLUGINS && (
                        <>
                            ...
                            {vxmpirePlugins.length > 0 || othersVisible.length > 0 ? (
                                <div className={cl("grid")}>
                                    {[...vxmpirePlugins, ...othersVisible]}
                                </div>
                            ) : (
                                <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                                    ...
                                    <div style={{ fontSize: 13 }}>{t("Add .tsx files to your")} <code>Documents/Vxmpire/userplugins/</code> {t("folder and rebuild.")}</div>
                                </div>
                            )}
                        </>
                    )}

                    {(searchValue.status !== SearchStatus.VXMPIRE && searchValue.status !== SearchStatus.OTHERS && searchValue.status !== SearchStatus.USER_PLUGINS) && (
                        <div className={cl("grid")}>
                            {vxmpirePlugins}
                            {othersVisible}
                        </div>
                    ...
                    {vxmpireData.length === 0 && othersData.length === 0 && searchValue.status === SearchStatus.LIKED_PLUGINS && (
                        <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                            ...
                            <div style={{ fontSize: 13 }}>{t("Like a Vxmpire plugin from its card to see it here.")}</div>
                        </div>
                    )}

                    {vxmpireData.length === 0 && othersData.length === 0 && searchValue.status !== SearchStatus.USER_PLUGINS && searchValue.status !== SearchStatus.LIKED_PLUGINS && (
                        <ExcludedPluginsList search={search} />
                    )}
