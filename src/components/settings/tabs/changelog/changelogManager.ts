...
const VXMPIRE_RELEASES_REPO = "vxmpire/vxmpire";
const VXMPIRE_REPO_URL = `https://source.${domain}/${VXMPIRE_RELEASES_REPO}`;
...
export { VXMPIRE_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Vxmpire, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(VXMPIRE_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
