...
import { ContributeModal } from "../../../../vxmpire/renderer/components/ContributeModal";
import { openNotificationSettingsModal } from "./NotificationSettings";

const cl = classNameFactory("vc-vencord-tab-");

const DEV_TEAM_IDS = [
    { id: "438492090121322498", role: "Owner" }
];
        ...
        <>
            <QuickActionCard>
                <QuickAction
                    Icon={GithubIcon}
                    text="Source Code"
                    action={() => VencordNative.native.openExternal("https://source.vxmpire.st/vxmpire/vxmpire")}
                />
                <QuickAction
                    Icon={PaintbrushIcon}
                    text="Edit QuickCSS"
                ...
                <QuickAction
                    Icon={PlanetIcon}
                    text="Vxmpire Channel"
                    action={() => VencordNative.native.openExternal("https://t.me/vxmpireoff")}
                />
            </QuickActionCard>
...
function useCompactActive() {
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("vxmpire-compact-change", handler);
        return () => window.removeEventListener("vxmpire-compact-change", handler);
    }, []);
    return active;
}
...
function useStealthActive() {
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("vxmpire-stealth-change", handler);
        return () => window.removeEventListener("vxmpire-stealth-change", handler);
    }, []);
    return active;
}
    ...
    return (
        <>
            ...
            <Paragraph className={Margins.bottom16}>
                {enabled
                    ? "Stealth mode is enabled â€” all Vxmpire visual elements are hidden. Shortcut: Ctrl+Shift+H"
                    : t("Hides all Vxmpire visual elements without disabling plugins. Shortcut: Ctrl+Shift+H")}
            </Paragraph>
            <Button
...
function MellowtelSupportSwitch() {
    ...
    return (
        <FormSwitch
            ...
            title={t("Share bandwidth to support Vxmpire (Mellowtel)")}
            description={
                consent
                    ? undefined
    ...
    return (
        <SettingsTab>
            ...
            {!stealthActive && (<>
                ...
                <Paragraph className={Margins.bottom16}>
                    {t("Configure how Vxmpire behaves and integrates with Discord. These settings affect the Discord client's appearance and behavior.")}
                </Paragraph>
                ...
                <Paragraph className={Margins.bottom16}>
                    {t("Configure how Vxmpire handles notifications. You can customize when and how you receive alerts, or view a history of past notifications.")}
                </Paragraph>
            ...
            <Paragraph className={Margins.bottom16}>
                {t("Replaces all Vxmpire buttons with a single compact toggle icon. Click the icon in the header bar, channel toolbar, or chat bar to restore all buttons.")}
            </Paragraph>
            <Button
...
export default wrapTab(EquicordSettings, "Vxmpire Settings");
