...
import { openVxmpireModal } from "@vxmpireplugins/compactMode/VxmpireModal";
    ...
    return (
        <Tooltip text={tooltip ?? ""} position={position} shouldShow={tooltip != null}>
            {({ onMouseEnter, onMouseLeave }) => (
                <Clickable
                    {...{ innerRef: ref } as any}
                    className={classes(HeaderBarClasses.clickable, "vxmpire-header-btn", className)}
...
try { _stealthActive = localStorage.getItem("Vxmpire_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Vxmpire_stealthMode", "1") : localStorage.removeItem("Vxmpire_stealthMode"); } catch { }
}

const NON_REACT_SELECTORS = [
    "#vxmpire-titlebar-btn",
    "#vxmpire-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("vxmpire-stealth"); else document.body?.classList.remove("vxmpire-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("vxmpire-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("vxmpire-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("vxmpire-stealth-change"));
}
...
try { _compactActive = localStorage.getItem("Vxmpire_compactMode") === "1"; } catch { }
...
function persistCompact(v: boolean) {
    try { v ? localStorage.setItem("Vxmpire_compactMode", "1") : localStorage.removeItem("Vxmpire_compactMode"); } catch { }
}

export function syncCompactBodyClass() {
    try {
        const stored = localStorage.getItem("Vxmpire_compactMode");
    ...
    try {
        if (_compactActive) {
            document.body?.classList.add("vxmpire-compact");
        } else {
            document.body?.classList.remove("vxmpire-compact");
        }
    } catch { }
...
export function toggleCompactMode() {
    ...
    _notifyCompactChange();
    try { if (_compactActive) document.body?.classList.add("vxmpire-compact"); else document.body?.classList.remove("vxmpire-compact"); } catch { }
    return _compactActive;
}

if (_compactActive) {
    try { document.body?.classList.add("vxmpire-compact"); } catch { }
}

export const compactListeners = new Set<() => void>();
export function _notifyCompactChange() {
    compactListeners.forEach(fn => fn());
    window.dispatchEvent(new Event("vxmpire-compact-change"));
}
...
function CompactSettingsPopout({ closePopout }: { closePopout: () => void; }) {
    ...
    useEffect(() => {
        ...
        window.addEventListener("vxmpire-compact-change", listener);
        window.addEventListener("vxmpire-stealth-change", listener);
        return () => {
            compactListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("vxmpire-compact-change", listener);
            window.removeEventListener("vxmpire-stealth-change", listener);
        };
    }, []);
        ...
        <div className="nc-settings-popout">
            ...
            <div className="nc-settings-popout-row" onClick={() => toggleStealthMode()}>
                <div className="nc-settings-popout-row-info">
                    ...
                    <div className="nc-settings-popout-row-desc">Hide all Vxmpire UI elements</div>
                </div>
...
function CompactHeaderBarToggle() {
    ...
    return (
        <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <HeaderBarButton
                icon={GearIcon}
                tooltip="Vxmpire Settings"
                onClick={() => openVxmpireModal()}
            />
        </div>
    );
}
...
export function _addHeaderBarButtons() {
    return [
        <style key="vxmpire-headerbar-style">{`
            .vxmpire-header-btn {
                display: flex;
                align-items: center;
                margin: 0 2px;
