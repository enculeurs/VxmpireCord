...
export type BadgeSource = "vencord" | "equicord" | "vxmpire" | "globalbadges" | "illegalcord";
...
export async function loadOwnHiddenBadgeSources(userId: string) {
    ...
    try {
        const localData = localStorage.getItem("vxmpire_hidden_badges");
        if (localData) {
    ...
    try {
        ...
        if (result?.config?.settings?.hidden !== undefined) {
            ...
            localStorage.setItem("vxmpire_hidden_badges", JSON.stringify(hidden));
        }
    } catch (e) {
