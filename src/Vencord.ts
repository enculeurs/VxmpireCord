...
function showGreenUpdateBanner() {
    if (document.getElementById("vxmpire-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "vxmpire-core-updater-root";
    ...
    titleSpan.textContent = "Vxmpire Update Available";
...
// @ts-ignore
window.showVxmpireUpdateBanner = showGreenUpdateBanner;
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Vxmpire update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Vxmpire", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Vxmpire.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    createAndAppendStyle("vxmpire-titlebar-position", coreStyleRootNode).textContent = `
        body:not(.vxmpire-stealth):not(.vxmpire-compact) [class*="title_c38"] {
