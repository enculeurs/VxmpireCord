...
import { openModal } from "@Vxmpire/types/utils";
import { onceReady } from "@Vxmpire/types/webpack";
import { VoiceActions } from "@Vxmpire/types/webpack/common";
...
import type SettingsPlugin from "@Vxmpire/types/plugins/_core/settings";
import { VesktopSettingsIcon } from "shared/icons";

VesktopLogger.log("read if cute :3");
VesktopLogger.log(`Vxmpire v${VesktopNative.app.getVersion()}`);

const { customEntries, customSections } = Vencord.Plugins.plugins.Settings as any as typeof SettingsPlugin;

customEntries.push({
    key: "Vxmpire_Vxmpire_settings",
    title: "Vxmpire Settings",
    Component: SettingsUi,
    Icon: VesktopSettingsIcon
});

customSections.push(() => ({
    section: "VxmpireSettings",
    label: "Vxmpire Settings",
    element: SettingsUi,
    className: "vc-Vxmpire-settings"
}));
...
const SECURITY_MODAL_KEY = "vxmpire_security_modal_seen";
onceReady.then(() => {
