...
// When Discord's OOP overlay crashes (always in Vxmpire — we're not discord.exe),
...
function isPopoutRateLimited(): boolean {
    ...
    if (popoutTimestamps.length >= POPOUT_RATE_LIMIT_MAX) {
        console.warn("[Vxmpire] Popout rate-limited — too many popout requests (overlay crash loop?)");
        return true;
    }
...
export function makeLinksOpenExternally(win: BrowserWindow) {
    win.webContents.setWindowOpenHandler(({ url, frameName, features }) => {
        console.log("[Vxmpire][LINK] setWindowOpenHandler url=", url, "frameName=", frameName);
        try {
        ...
        if (isDiscordPopout || (frameName.startsWith("DISCORD_") && pathname === "/popout" && DISCORD_HOSTNAMES.includes(hostname))) {
            ...
            // The game overlay can never work in Vxmpire (wrong process name),
            // so silently deny these instead of letting them flood.
            if (OVERLAY_FRAME_NAMES.has(frameName)) {
                console.log("[Vxmpire] Blocked overlay popout (overlay unsupported):", frameName);
                return { action: "deny" };
            }
    ...
    win.webContents.on("did-create-window", (childWin, { frameName, options, url }: any) => {
        console.log("[Vxmpire][LINK] did-create-window url=", url, "frameName=", frameName);
