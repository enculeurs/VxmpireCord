...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Vxmpire Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Vxmpire's recommended Steam Deck controller settings?
