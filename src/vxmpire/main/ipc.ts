...
handleSync(IpcEvents.GET_GIT_HASH, () => Vxmpire_GIT_HASH);
...
// Handler pour VencordNative.vxmpire.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.VxmpireDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { VxmpireDir } = State.store;
    if (!VxmpireDir) return;

    const stats = await stat(VxmpireDir);
    if (!stats.isDirectory()) return;

    shell.openPath(VxmpireDir);
});
