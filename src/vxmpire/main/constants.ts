...
const VxmpireDir = dirname(process.execPath);

export const PORTABLE =
    process.platform === "win32" &&
    !process.execPath.toLowerCase().endsWith("electron.exe") &&
    !existsSync(join(VxmpireDir, "Uninstall Vxmpire.exe"));

export const DATA_DIR =
    process.env.Vxmpire_USER_DATA_DIR || (PORTABLE ? join(VxmpireDir, "Data") : join(app.getPath("userData")));
...
export const USER_AGENT = `Vxmpire/${app.getVersion()} (https://git.${domain}/vxmpire/vxmpire)`;
