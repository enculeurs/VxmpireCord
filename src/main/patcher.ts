...
console.log("[Vxmpire] Starting up...");
...
if (!IS_VANILLA) {
    ...
    class BrowserWindow extends electron.BrowserWindow {
        constructor(options: BrowserWindowConstructorOptions) {
            // On n'injecte le preload Vxmpire QUE dans les fenêtres Discord/Vxmpire légitimes.
            ...
            // le titre est une fenêtre Vxmpire/Equicord/Discord connue.
            const KNOWN_TITLES = /^(Discord|Vesktop|Equibop)$|^(Vxmpire|Equicord)/;
            ...
            if (options?.webPreferences?.preload && (isTrustedTitle || isVBCable || preloadIsOurs)) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Vxmpire] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Vxmpire] setBackgroundMaterial failed:", e);
                    }
                }
                ...
                // est géré exclusivement par vxmpire-index.js (app.on("web-contents-created"))
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Vxmpire");
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[Vxmpire] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
    ...
    console.log("[Vxmpire] Running in vanilla mode. Not loading Vxmpire");
}

console.log("[Vxmpire] Loading original Discord app.asar");
