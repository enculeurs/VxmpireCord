        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Vxmpire] Installed React Developer Tools"))
                    .catch(err => console.error("[Vxmpire] Failed to install React Developer Tools", err));
        } catch { }

        initCsp();

        applyStoredMellowtelConsent().catch(err =>
            console.error("[Vxmpire] Failed to apply stored Mellowtel consent", err)
        );
    });
}
