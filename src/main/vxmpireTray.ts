...
/**
 * Initializes the Vxmpire system tray icon.
 ...
 */
export function initVxmpireTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall Vxmpire",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "vxmpire", "Uninstall Vxmpire.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Vxmpire", "Uninstall Vxmpire.exe"),
                            join(process.env.PROGRAMFILES || "", "Vxmpire", "Uninstall Vxmpire.exe"),
                        ];
            ...
            console.log("[Vxmpire] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Vxmpire] Failed to create tray:", e);
        }
    });
}
