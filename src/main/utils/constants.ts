...
export const DATA_DIR = process.env.VXMPIRE_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "VxmpireData", suffix)
        : join(app.getPath("userData"), "..", "Vxmpire", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Vxmpire] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
