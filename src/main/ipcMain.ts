...
import { makeLinksOpenExternally } from "../vxmpire/main/utils/makeLinksOpenExternally";
...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "vxmpire-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "vxmpire-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "vxmpire-wbs-"));
...
const OFFICIAL_UPDATE_URL = `https://git.${domain}/vxmpire/vxmpire/releases/download/latest/Vxmpire-Installer.exe`;

ipcMain.handle(IpcEvents.VXMPIRE_DOWNLOAD_AND_RUN, async (event, url: string) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    ...
    const tmpPath = path.join(os.tmpdir(), "VxmpireUpdate-Setup.exe");
    ...
    const { response } = await dialog.showMessageBox({
        ...
        defaultId: 0,
        title: "Vxmpire Update",
        message: "A Vxmpire update is available.",
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async (event) => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Vxmpire-VBCable");
    ...
    try {
        ...
        console.error("[Vxmpire] VBCable install failed:", err);
