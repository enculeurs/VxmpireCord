...
const API_BASE      = `${GITEA_BASE}/api/v1/repos/vxmpire/vxmpire`;
const REPO_URL      = `${GITEA_BASE}/vxmpire/vxmpire`;
declare const VERSION: string;
const CURRENT_VERSION = `v${VERSION}`;
const ZIP_FILE = "vxmpire-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `vxmpire-update-${Date.now()}.zip`);
        ...
        const tmpExtract = join(app.getPath("temp"), `vxmpire-extract-${Date.now()}`);
