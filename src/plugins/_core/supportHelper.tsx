...
import { platformName } from "@vxmpireplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Vxmpire developers"
        const vxmpireGroupIds = [
            "438492090121322498"
        ];

        const isOriginalDev = originalDevIds.includes(userId);
        const isVxmpireGroup = vxmpireGroupIds.includes(userId);

        if (!isOriginalDev && !isVxmpireGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Vxmpire developers";
