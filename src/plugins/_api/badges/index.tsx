...
let VxmpireBadges = {} as Record<string, Array<{ icon: string; placeholder: string; uuid: string; }>>;
...
async function loadAllBadges(noCache = false) {
    ...
    const vxmpireBadges = await loadBadges(`https://api.${domain}/badges`, noCache).catch(() => ({}));
...
export default definePlugin({
    ...
    getVxmpireBadges(userId: string) {
        try {
            const userBadges = VxmpireBadges[userId];
            ...
            return userBadges
                ...
                .map(badge => ({
                    iconSrc: badge.icon,
                    description: badge.placeholder ?? "Vxmpire Badge",
                    position: BadgePosition.START,
                    props: {
                        style: {
                            borderRadius: "50%",
                    ...
                    onClick() {
                        return GenericBadgeModal(badge, "Vxmpire");
                    }
                } satisfies ProfileBadge));
        } catch (e) {
            console.error("[BadgeAPI] Error processing vxmpire badges for", userId, e);
            return [];
        }
    },
