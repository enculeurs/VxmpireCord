@echo off
title Vxmpire Installer - Build
cd /d "%~dp0"

echo.
echo  ================================
echo   Vxmpire Installer - Build
echo  ================================
echo.
...
:: Build electron-builder -> Vxmpire-Installer.exe dans ../release/installer/
echo.
...
:: Verification
if not exist "release\installer\Vxmpire-Installer.exe" (
    echo.
    echo  [ERREUR] Vxmpire-Installer.exe introuvable apres build.
    pause
    exit /b 1
)

for %%F in ("release\installer\Vxmpire-Installer.exe") do (
    echo.
    echo  [OK] Build reussi ^!
    echo  Fichier : release\installer\Vxmpire-Installer.exe  (%%~zF octets^)
    echo.
)
