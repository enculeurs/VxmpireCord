...
**https://vxmpire.st/privacy** — that page is the canonical version; if anything here ever
conflicts with it, the website version applies.

## Summary

Vxmpire is a client modification for Discord. Most functionality (plugins, themes, settings,
keybinds) runs entirely on your device and is never sent to us.

A small set of **opt-in** features talk to our backend at `api.vxmpire.st`:
...
- Community-role cosmetic badges — your Discord user ID plus the roles you hold in the official
  Vxmpire server, used only to grant cosmetic badges.
- Username-history lookups — on demand only, forwards the looked-up Discord user ID to a
  third-party username-history provider.

If you never sign in with Discord inside Vxmpire, none of the above applies to you.
...
Since Vxmpire modifies the official Discord client, all messaging, calls, and server activity
...
details), see **https://vxmpire.st/privacy**.
