# build-installer.ps1 — Build Vxmpire-Installer.exe (Electron + electron-builder)
...
$OutExe    = Join-Path $OutDir "Vxmpire-Installer.exe"

Write-Host ""
Write-Host "  [Vxmpire] Building Electron installer..." -ForegroundColor Cyan
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  Vxmpire-Installer.exe compile ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERREUR] Vxmpire-Installer.exe introuvable apres compilation." -ForegroundColor Red
    exit 1
}
